...
namespace lib {
     ...
     */
    export function start(completeFunc, params) {
        if (hasStart) {
        ...
        if (Platform.type == "creator") {
            data = DataManager.getInstance()._root.black;
            Platform.start();
    ...
    export function $error(errorCode, ...args) {
        var msg;
    ...
    export function $warn(errorCode, ...args) {
        var msg;
        if (typeof errorCode == "string") {
            msg = errorCode;
        } else {
    ...
    export function $tip(errorCode, ...args) {
...
namespace mvc {
    ...
    export class View {
        ...
        public registerModule(module: Module): void {
            ...
            if (interests.length) {
                var observer = new Observer(module.handleNotification, module);
                for (let i = 0, len = interests.length; i < len; i++) {
                    this.registerObserver(interests[i], observer);
                }
            }
         ...
         * @returns {mvc.Module}
         */
        public removeModule(moduleName: string): Module {
...
namespace mvc {
    ...
    export abstract class Mediator extends Notifier {
        ...
        public load(): Promise<void> {
            return new Promise<void>(this.asyncLoad.bind(this));
        }

        protected asyncLoad(resolve: Function): void {
...
namespace mvc {
    ...
    export abstract class Module extends Notifier {

        private _name: string;
        private _config: any;

        constructor(name: string, config: any = null) {
            super();
            this._name = name || Mediator.NAME;
            this._config = config;
        }
